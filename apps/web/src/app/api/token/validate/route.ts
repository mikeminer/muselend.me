...
export async function POST(request: Request) {
  ...
  try {
    ...
    const version = await client.readContract({ address: validator, abi: CreatorTokenValidatorAbi, functionName: "canonicalVersion", args: [token] });
    const valid = version > 0 && await client.readContract({ address: validator, abi: CreatorTokenValidatorAbi, functionName: "validate", args: [token, version] });
    return NextResponse.json(
